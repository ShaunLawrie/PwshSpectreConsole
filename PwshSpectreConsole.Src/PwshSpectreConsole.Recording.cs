...
internal sealed class AsciiCastWriter : TextWriter {
...
public class AsciiCastInput : IAnsiConsoleInput {
    ...
    public void PushCharacter(char input, int keypressDelayMs) {
        int delay = keypressDelayMs + _random.Next((int)(keypressDelayMs * -.2), (int)(keypressDelayMs * .2));
        ...
        switch (input) {
            ...
            case '¦':
                ...
                break;
            default:
                bool control = char.IsUpper(input);
    ...
    public ConsoleKeyInfo? ReadKey(bool intercept) {
        ...
        (ConsoleKeyInfo?, int) result = _input.Dequeue();
    ...
    public int KeyEntries { get; private set; }
}

public partial class RecordingConsole : IAnsiConsole {
    private readonly IAnsiConsole _ansiConsole;
    private readonly AsciiCastWriter _writer;
    ...
    public string GetAsciiCastRecording(string title) {
        ...
        json = Newline().Replace(json, @"\r\n");
        json = DoubleNewline().Replace(json, @"\r\n");
        // count number of times [2A appears in the json
        MatchCollection cursorUps = CursorUps().Matches(json);
        int countOfCursorUps = 0;

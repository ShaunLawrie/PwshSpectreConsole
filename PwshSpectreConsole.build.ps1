...
$script:config = @{
    ...
    SourcePath       = Join-Path $PSScriptRoot 'PwshSpectreConsole.Src'
    OutputPath       = Join-Path $PSScriptRoot 'output'
    ModuleSourcePath = Join-Path $PSScriptRoot 'PwshSpectreConsole'
    TestPath         = Join-Path $PSScriptRoot 'PwshSpectreConsole.Tests'
    CsprojPath       = Join-Path $PSScriptRoot 'PwshSpectreConsole.Src' 'PwshSpectreConsole.csproj'
...
function MergePSM1 {
    ...
    '#region {0}' -f ([System.IO.Path]::GetRelativePath($PSScriptRoot, $Path.FullName) -replace '\\','/')
